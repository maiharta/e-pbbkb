...
<div class="page-heading">
    ...
    <section class="section">
        <div class="card">
            <div class="card-body">
                <form
                    action="{{ route('pelaporan.penjualan.update', ['penjualan' => $penjualan->ulid, 'ulid' => $pelaporan->ulid]) }}"
                    method="POST">
                    @csrf
                    @method('PUT')
                    <x-input.text label="Pembeli" name="pembeli" placeholder="Masukkan nama pembeli"
                        value="{{ old('pembeli', $penjualan->pembeli) }}" />
                    <x-input.text label="Alamat" name="alamat" placeholder="Masukkan nama alamat"
                        value="{{ old('alamat', $penjualan->alamat) }}" />
                    <x-input.select
                        :options="$kabupatens->map(fn($item) => ['key' => $item->id, 'value' => $item->nama])"
                        label="Kabupaten" name="kabupaten_id" placeholder="Pilih kabupaten"
                        value="{{ old('kabupaten_id', $penjualan->kabupaten_id) }}" />
                    <x-input.select :options="$sektors->map(fn($item) => ['key' => $item->id, 'value' => $item->nama])"
                        label="Sektor" name="sektor_id" placeholder="Pilih sektor"
                    ...
                    <x-input.select :options="$jenis_bbms->map(
                        ...
                        )" label="Jenis BBM" name="jenis_bbm_id" placeholder="Pilih jenis BBM"
                        value="{{ old('jenis_bbm_id', $penjualan->jenis_bbm_id) }}" />
                    <div class="form-group mb-3">
                        <label class="col-form-label fw-bold" for="volume">Total Volume (Liter)</label>
                        <input autocomplete="off" class="form-control" id="volume" name="volume"
                            placeholder="Masukkan total volume" type="text"
                            value="{{ old('volume', (int) $penjualan->volume) }}">
                    </div>
                    <x-input.number :is_currency="true" label="DPP" name="dpp" placeholder="Masukkan DPP"
                    ...
                    <x-input.number :currency="true" :is_currency="true" label="PBBKB" name="pbbkb"
                        placeholder="Masukkan total PBBKB" value="{{ old('pbbkb', $penjualan->pbbkb) }}" />
                    <x-input.select
                        :options="[['key' => 'depot', 'value' => 'Depot'], ['key' => 'TBBM', 'value' => 'TBBM']]"
                        label="Lokasi Penyaluran" name="lokasi_penyaluran" placeholder="Pilih lokasi penyaluran"
                    ...
                    <x-input.select :options="[
                        ...
                        ]" label="Status Pajak" name="is_wajib_pajak" placeholder="Pilih status pajak pembeli"
